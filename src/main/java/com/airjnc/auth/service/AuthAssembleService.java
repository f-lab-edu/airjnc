...
/**
 * 만약, 어떠한 기능이 2개 이상의 `Service` 들을 모아서 만들어야 한다면 `XXXAssembleService`를 이용한다. 1. `XXXAssembleService`는 `XXXService` 를 의존할
 * 수 있으나, `XXXService`는 다른 `YYYService`를 의존할 수 없다. 순환성 참조를 막기 위해 이러한 규칙을 세웠다.
 */
@Service
