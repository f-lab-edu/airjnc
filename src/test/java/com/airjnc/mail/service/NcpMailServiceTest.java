...
import com.airjnc.common.service.NcpBaseService;
...
class NcpMailServiceTest {
  ...
  void send() {
    ...
    given(ncpBaseService.createHeaders(sendUri)).willReturn(headers);
    ...
    //when
    SendUsingTemplateDto sendUsingTemplateDto = SendUsingTemplateDto.builder().name(TestUser.NAME).code("123456").build();
    ncpMailService.send(TestUser.EMAIL, sendUsingTemplateDto);
    //then
    then(ncpBaseService).should(times(1)).createHeaders(sendUri);
