...
class UserControllerTest {
    ...
    @Test
    @DisplayName("올바른_이름_번호-이메일 찾기 성공")
    public void whenValidNameAndPhoneNumberThenSuccessFindEmail() throws Exception {
        // given
        FindEmailResponseDTO findEmailResponseDTO = FindEmailResponseDTO.builder().email(UserFixture.EMAIL).build();
        BDDMockito.given(userService.findEmailByNameAndPhoneNumber(findEmailRequestDTO)).willReturn(findEmailResponseDTO);
        // when,then
        mvc.perform(get("/user/login/findemail").param("name", findEmailRequestDTO.getName()).param("phoneNumber", findEmailRequestDTO.getPhoneNumber()))
            .andDo(print())
            .andExpect(jsonPath("$.email").value(findEmailResponseDTO.getEmail()));
    }


    @Test
    @DisplayName("올바르지않은_이름_번호-이메일 찾기 실패")
    public void whenInvalidNameAndPhoneNumberThenFailFindEmail() throws Exception {
        // given
        BDDMockito.given(userService.findEmailByNameAndPhoneNumber(findEmailRequestDTO)).willThrow(new FindEmailNotMatchException());
        // when,then
        mvc.perform(get("/user/login/findemail").param("name", findEmailRequestDTO.getName()).param("phoneNumber", findEmailRequestDTO.getPhoneNumber()))
            .andDo(print())
            .andExpect(status().is4xxClientError());
